...
const RegistrationPage = () => {
  ...
  const validateForm = () => {
    if (!formData.anrede || !formData.vorname || !formData.nachname || !formData.email || 
        !formData.arbeitsstelle_name || !formData.arbeitsstelle_adresse || 
        !formData.arbeitsstelle_stadt || !formData.arbeitsstelle_plz || 
        !formData.arbeitsstelle_land || !formData.taetigkeitsbereich) {
    ...
    if (formData.taetigkeitsbereich === 'Sonstiges' && !formData.taetigkeitsbereich_sonstiges) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(error.response?.data?.message || 'Ein Fehler ist bei der Registrierung aufgetreten.');
    }    
  };
    ...
    <div className="container mt-5">
      ...
      <form onSubmit={handleSubmit} className="mt-4">
        ...
        <div className="form-group mb-3">
          ...
          <select className="form-control" id="titel" name="titel" value={formData.titel} onChange={handleChange}>
        ...
        <div className="form-group mb-3">
          ...
          <select className="form-control" id="taetigkeitsbereich" name="taetigkeitsbereich" value={formData.taetigkeitsbereich} onChange={handleChange}>
            ...
            <option value="Arzneimittelentwicklung">Arzneimittelentwicklung</option>
        ...
        <div className="form-group form-check mb-3">
          <input type="checkbox" className="form-check-input" id="datenschutzCheckbox" required />
          <label className="form-check-label" htmlFor="datenschutzCheckbox">
            Ich bin einverstanden, dass meine Daten zur Bearbeitung der Anfrage zur Nutzung des Flip-Flop-Rechners gespeichert werden dürfen. Die Informationen zum Datenschutz habe ich gelesen (<a href="https://ck-care.ch/datenschutz/">DATENSCHUTZ | CK-CARE</a>).
          </label>
        </div>
  
        <button type="submit" className="btn btn-primary mb-3">Abschicken</button>
      </form>
    </div>
  );  
  };
